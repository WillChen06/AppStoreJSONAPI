...
class AppDetailController: BaseListController {
    ...
    private func fetchDetail() {
        Task {
            async let res: SearchResult? =  Service.shared.fetchApp(by: appId)
            async let reviews: Reviews? = Service.shared.fetchAppReviews(appId: appId)

            self.app = await res?.results.first
